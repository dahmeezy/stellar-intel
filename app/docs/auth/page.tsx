...
export default function AuthPage() {
      ...
      <section className="space-y-4">
        ...
        <div className="rounded-xl border border-border p-5">
          ...
          <ul className="mt-2 space-y-1 text-sm text-secondary-text">
            <li><code className="text-accent">GET /api/rates/[corridor]</code></li>
            <li><code className="text-accent">POST /api/intent/offramp</code></li>
            <li><code className="text-accent">GET /api/reputation/[anchor]</code></li>
            <li><code className="text-accent">GET /api/reputation/leaderboard</code></li>
            <li><code className="text-accent">GET /api/publisher/health</code></li>
            <li><code className="text-accent">GET /api/snapshot</code></li>
            <li><code className="text-accent">GET /v1/public/scores</code></li>
          </ul>
        </div>
        ...
        <div className="rounded-xl border border-border p-5">
          ...
          <p className="mt-1 text-sm text-secondary-text">
            Admin endpoints require the <code>X-Admin-Key</code> header set to the <code>ADMIN_SECRET_KEY</code> environment variable.
          </p>
          <CodeBlock
            language="bash"
  ...
  -H 'x-admin-key: YOUR_ADMIN_SECRET_KEY'`}
          />
          <ul className="mt-2 space-y-1 text-sm text-secondary-text">
            <li><code className="text-accent">GET /api/admin/disputes</code></li>
            <li><code className="text-accent">POST /api/admin/disputes</code></li>
          </ul>
        </div>
        ...
        <div className="rounded-xl border border-border p-5">
          ...
          <p className="mt-1 text-sm text-secondary-text">
            Internal cron-triggered endpoints require an <code>Authorization: Bearer CRON_SECRET</code> header.
          </p>
          <CodeBlock
            language="bash"
  ...
  -H 'authorization: Bearer YOUR_CRON_SECRET'`}
          />
          <ul className="mt-2 space-y-1 text-sm text-secondary-text">
            <li><code className="text-accent">POST /api/publisher/tick</code></li>
            <li><code className="text-accent">POST /api/reputation/reconcile</code></li>
            <li><code className="text-accent">POST /api/reputation/refresh</code></li>
          </ul>
        </div>
        ...
        <div className="rounded-xl border border-border p-5">
          ...
          <p className="mt-1 text-sm text-secondary-text">
            The dispute endpoint uses <strong>Ed25519 cryptographic signature verification</strong>. The request body includes a signature over the <code>intentHash</code> that the server verifies before accepting the dispute.
          </p>
          <CodeBlock
            language="bash"
    ...
    "reason": "Rate did not match quoted amount"
      ...
      <section className="space-y-4">
        ...
        <p className="text-secondary-text">
          Rate limiting is applied on a per-IP basis using a sliding window algorithm.
        ...
        <div className="overflow-x-auto rounded-xl border border-border">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border">
              <tr>
                <td className="px-4 py-3 font-mono text-xs text-accent">GET /api/rates/[corridor]</td>
                <td className="px-4 py-3 text-secondary-text">90 requests</td>
                <td className="px-4 py-3 text-secondary-text">60 seconds</td>
              </tr>
              <tr>
                <td className="px-4 py-3 font-mono text-xs text-accent">POST /api/intent/offramp</td>
                <td className="px-4 py-3 text-secondary-text">20 requests</td>
                <td className="px-4 py-3 text-secondary-text">60 seconds</td>
              </tr>
              <tr>
                <td className="px-4 py-3 font-mono text-xs text-accent">POST /api/reputation/dispute</td>
              ...
              <tr>
                <td className="px-4 py-3 font-mono text-xs text-accent">GET /api/publisher/health</td>
        ...
        <div className="rounded-xl border border-border bg-bg-subtle p-5">
          ...
          <p className="mt-1 text-sm text-secondary-text">
            When a rate limit is exceeded, the API returns a <code>429 Too Many Requests</code> response:
          </p>
          <CodeBlock
            language="json"
            code={`{
  ...
  "message": "Rate limit exceeded. Retry after 45 seconds."
          ...
          <p className="mt-2 text-sm text-secondary-text">
            The response includes a <code>Retry-After</code> header with the number of seconds to wait, and <code>X-RateLimit-Remaining</code> headers on successful requests.
          </p>
        </div>
      </section>
      ...
      <section className="space-y-4">
        ...
        <ul className="space-y-2 text-sm text-secondary-text">
          ...
          <li>• Use the <code>Cache-Control</code> headers returned by the API</li>
          <li>• Consider using the snapshot endpoint for periodic polling instead of the live rates endpoint</li>
        </ul>
      </section>
    </div>
  );
